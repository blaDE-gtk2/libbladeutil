 ...
 *
 * The returned string is owned by libbladeutil and must not be freed by
 * the caller.
 *
 ...
 *
 * The returned string is managed by libbladeutil and must not be freed by
 * the caller.
 *
...
{
  if (g_ascii_isspace (*p) /* fmb ~/music */
